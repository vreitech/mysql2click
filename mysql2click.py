                ...
                if conf_def.getboolean("make_mysql_delete"):
                    ...
                    try:
                        ...
                        if e.args[0] == 1205:
                            ...
                            logger.warning('Работа скрипта будет продолжена, однако следует проверить, что в MySQL нет XA-транзакций, блокирующих удаление, командой "XA RECOVER;".')
                            logger.warning('Текст исключения представлен ниже.')
                            logger.exception(sys.exc_info()[0])
                        else:
                            logger.exception(sys.exc_info()[0])
                            sys.exit(17)
